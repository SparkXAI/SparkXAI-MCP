...
当前版本 v1.3.0 支持数据查询、AI 托管组管理、SP Campaign 创建和 Amazon Sponsored Ads 广告实体编辑。可创建包含广告组、推广商品、投放与广告位设置的 SP Campaign，也可修改 Campaign 预算、状态和 SP 竞价策略，管理关键词与 Target 的状态和竞价，以及推广商品和否定投放。暂不支持创建 SB/SD Campaign 或使用独立删除操作，但支持通过状态更新归档 Campaign。
...
- **创建 SP Campaign**——创建包含广告组、推广商品、自动/关键词/商品投放、否定投放和广告位设置的 SP Campaign，执行前先展示完整结构。
- **编辑线上广告实体**——修改 Campaign 预算、状态、SP 竞价策略、广告位加价和 AMC 受众加价，调整关键词或 Target 竞价，并管理推广商品和否定投放。写操作会直接在线上账号生效，执行前必须确认对象和具体变更。
...
| **广告实体管理** | 创建 SP Campaign；修改 Campaign 预算、状态、SP 竞价策略、广告位加价和 AMC 受众加价；管理关键词和 Target 的状态与竞价；归档推广商品；创建、复制或更新支持的否定投放 |
...
4. 直接安装 8 个必装 Skills：3 个查询 Skills，以及 `sparkx-create-ai-group`、`sparkx-edit-ai-group`、`sparkx-delete-ai-group`、`sparkx-edit-ads`、`sparkx-create-campaign`，无需再次询问用户。
...
**必装（8 个）**——基础查询、托管组管理、SP Campaign 创建和广告实体编辑能力：
...
| [sparkx-create-campaign](skills/sparkx-create-campaign/) | `create_sp_sb_campaign` | 创建 SP Campaign，包括广告组、推广商品、自动/关键词/商品投放、否定投放、广告位和可选 AMC 受众设置 |
...
- **Claude Code / Codex / Cursor 等**：把 [`skills/`](skills/) 目录发给 AI，说一句"帮我把 8 个必装 Skill 装上"，可选 Skills 按需加装。
...
- 「为这些 ASIN 创建一个暂停状态的 SP Campaign，先展示完整结构让我确认。」
...
- **写入范围**：当前支持 AI 托管组管理、SP Campaign 创建和部分广告实体编辑。可创建 SP Campaign，并修改 Campaign 预算、状态、SP 竞价策略、广告位加价和 AMC 受众加价，包括通过状态更新归档 Campaign；暂不支持创建 SB/SD Campaign 或使用独立删除操作。
- **暂不支持**：词库相关设置；可复用模板本身的创建与编辑；RBA 规则配置的修改（**可以读取**）。现有模板可以读取，并可用于创建或编辑托管组。托管组排期已支持读写，但仅限 SP/SB。行动空间允许从 RBA 切换为 AI，但不支持从 AI 切换为 RBA。
...
4. 找到并安装 **SparkX AI MCP**。Plugin 会同时安装 MCP server 和全部 12 个 Skills。
...
5. 新建一个 Codex 任务，MCP server 和全部 12 个 Skills 将在新任务中可用。
...
Plugin 会同时安装 MCP server 和全部 12 个 Skills。当前 Plugin 配置使用 `SPARKX_AI_TOKEN`；如需使用 OAuth，请按[安装说明中的 OAuth 配置](docs/installation.md#oauth)连接。
